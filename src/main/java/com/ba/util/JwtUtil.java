...
public class JwtUtil {
    ...
    // this method is for generating token. as argument is tc. so as user first time send request with tc and password
    // so here we will fetch the tc , so based on that tc we are going to create one token
    public String generateToken(String tc) {
        Map<String, Object> claims = new HashMap<>();
        return createToken(claims, tc);
    }

    // in this method createToken subject argument is tc
    // here we are setting the time for 10 hours to expire the token.
    // and you can see we are using HS256 algorithmn
    // TOKENIN SURESI BURADAN AYARLANIYOR.
    ...
    public Boolean validateToken(String token, UserDetails userDetails) {
        ...
        final String TC = extractUsername(token);
